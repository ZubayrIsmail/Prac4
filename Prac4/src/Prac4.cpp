...
int setup_gpio(void){
    ...
    wiringPiSPISetup (chan, SPI_SPEED) ; 
...
void *playThread(void *threadargs){
	...
	while (!playing)
	   continue;
        ...
        //TODO
	wiringPiSPIDataRW(chan, buffer[bufferReading][buffer_location], 2);
...
int main(){
	...
	//First 8 bits consists of 4 control bits and the first 2 bits of the character
        buffer[bufferWriting][counter][0] = 0b00110000|(ch>>6);
        //Set next 8 bit packet
	//Next 8 bits consists of the last 6 bits of the character
